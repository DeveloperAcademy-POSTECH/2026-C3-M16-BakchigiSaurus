...
nonisolated struct PlayerID: Codable, Hashable, Identifiable, Comparable {
    ...
    init(stablePeerRawID rawID: String) {
        self.rawValue = Self.makeStableUUID(from: rawID)
    }
    ...
    private static func makeStableUUID(from rawID: String) -> UUID {
        let digest = Array(SHA256.hash(data: Data(rawID.utf8)).prefix(16))
        var bytes = digest
